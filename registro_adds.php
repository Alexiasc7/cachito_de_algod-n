...
<script>
	...
	{   
		...
		{
			window.location = "registro_adds.php?ideliminar=" + id;
		}
  ...
  function modificar(id)
	{
        if (confirm("¿ Estas seguro de modificar el registro ?")) {
            window.location = "registro_adds.php?id_anuncio=" + id; 
        }
	}
...
//REALIZAMOS UNA PETICION SQL AL SERVER A TRAVES DEL OBJETO//
    ...
    if(isset($_POST['botoninsertar']))
    {
        ...
        $obj->Ejecutar_Instruccion("UPDATE `anuncios` SET `titulo` = '$nombre', `imagen` = '$fichero_subido' WHERE id_anuncio = '$id_anuncio';");
            
        header("location: registro_adds.php");
        }
        else if ( $_POST['botoninsertar']=='Insertar') 
        {
            ...
            $obj->Ejecutar_Instruccion("INSERT INTO `anuncios` (`titulo`, `imagen`)  
    ...
    else if(isset($_GET['ideliminar']))
    {
        ...
        $obj->Ejecutar_Instruccion("DELETE FROM anuncios WHERE id_anuncio='$eliminar'");
        header("location: registro_adds.php"); 
        
    }
    elseif (isset($_GET['id_anuncio'])) 
    {		
                $idmodificar = $_GET['id_anuncio'];
                $addmod = $obj->Ejecutar_Instruccion("SELECT * FROM anuncios WHERE id_anuncio = '$idmodificar'");
    }
    ...
    if(isset($_POST['botonbuscar']))
    {
        ...
        $result = $obj->Ejecutar_Instruccion("Select * from anuncios where titulo like '%$buscar%'");
        
    }
    else
    {
        $result=$obj->Ejecutar_Instruccion("Select * from anuncios");
    }
?>
    <div class="header">
        <div class="container">
                        ...
                        <div class="row">
                          <div id="form-login" class="col-md-6 col-md-offset-3">
                            <div class="section-heading">
                              <h4>Registro Anuncios</h4>
                              <div class="line-dec"></div>
                              <br>
                              <br>
                            </div>
                            <form class="searchable" action="registro_adds.php" method="post" id="formularioinsertar" name="formularioinsertar" enctype="multipart/form-data">                         
                              ...
                              <div class="col-md-12">
                                <fieldset>
                                  ...
                                  <input type="submit" name="botoninsertar" class="btn btn-success" id="botoninsertar"  value="<?php echo isset($_GET['id_anuncio']) ? 'Modificar' : 'Insertar'; ?>">
                                </div>                            
                                </fieldset>
                              </div>
                            </form>
